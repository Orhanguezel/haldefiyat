...
release=.next-release-20260914bc01
test ! -e "frontend/$release"
old_target=$(readlink frontend/standalone-server.js)
mapfile -t files < <(tar -tf /tmp/hal-gzl-banner-20260914.tar)
sha256sum -c /tmp/hal-gzl-banner-before.sha256
mkdir -p /tmp/hal-gzl-banner-backup-20260914
for path in "${files[@]}"; do
  if test -f "$path"; then cp --parents "$path" /tmp/hal-gzl-banner-backup-20260914/; fi
done
printf '%s\n' "$old_target" > /tmp/hal-gzl-banner-backup-20260914/old-target.txt
tar -xf /tmp/hal-gzl-banner-20260914.tar
restore_link() { ln -sfn "$old_target" frontend/standalone-server.js; }
trap restore_link ERR
(cd frontend && NEXT_DIST_DIR="$release" NODE_OPTIONS=--max-old-space-size=3072 bun run build) > /tmp/hal-gzl-banner-build.log 2>&1
...
(cd backend && bun scripts/gzl-technology-banner-20260914.ts)
