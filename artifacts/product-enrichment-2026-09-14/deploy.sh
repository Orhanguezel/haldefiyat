...
release=.next-release-20260914ba01
test ! -e "frontend/$release"
old_target=$(readlink frontend/standalone-server.js)
mapfile -t files < <(tar -tf /tmp/hal-product-enrichment-20260914.tar)
for path in "${files[@]}"; do
  test -z "$(git diff -- "$path")" || { echo "Concurrent edit: $path"; exit 1; }
done
mkdir -p /tmp/hal-product-enrichment-backup-20260914
for path in "${files[@]}"; do
  if test -f "$path"; then cp --parents "$path" /tmp/hal-product-enrichment-backup-20260914/; fi
done
printf '%s\n' "$old_target" > /tmp/hal-product-enrichment-backup-20260914/old-target.txt
tar -xf /tmp/hal-product-enrichment-20260914.tar
restore_link() { ln -sfn "$old_target" frontend/standalone-server.js; }
trap restore_link ERR
(cd frontend && NEXT_DIST_DIR="$release" NODE_OPTIONS=--max-old-space-size=3072 bun run build) > /tmp/hal-product-enrichment-build.log 2>&1
