...
release=.next-release-20260915bc01
test ! -e "frontend/$release"
old_target=$(readlink frontend/standalone-server.js)
mapfile -t files < <(tar -tf /tmp/hal-listing-rules.tar)
mkdir -p /tmp/hal-listing-rules-backup-20260915
for path in "${files[@]}"; do
  if test -f "$path"; then cp --parents "$path" /tmp/hal-listing-rules-backup-20260915/; fi
done
printf '%s\n' "$old_target" > /tmp/hal-listing-rules-backup-20260915/old-target.txt
tar -xf /tmp/hal-listing-rules.tar
pm2 reload hal-backend
restore_link() { ln -sfn "$old_target" frontend/standalone-server.js; }
trap restore_link ERR
(cd frontend && NEXT_DIST_DIR="$release" NODE_OPTIONS=--max-old-space-size=3072 bun run build) > /tmp/hal-listing-rules-build.log 2>&1
