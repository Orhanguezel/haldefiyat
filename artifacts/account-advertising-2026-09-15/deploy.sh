...
release=.next-release-20260915ac01
test ! -e "frontend/$release"
old_target=$(readlink frontend/standalone-server.js)
mapfile -t files < <(tar -tf /tmp/hal-account-ads.tar)
mkdir -p /tmp/hal-account-ads-backup-20260915
for path in "${files[@]}"; do
  if test -f "$path"; then cp --parents "$path" /tmp/hal-account-ads-backup-20260915/; fi
done
printf '%s\n' "$old_target" > /tmp/hal-account-ads-backup-20260915/old-target.txt
tar -xf /tmp/hal-account-ads.tar
restore_link() { ln -sfn "$old_target" frontend/standalone-server.js; }
trap restore_link ERR
(cd frontend && NEXT_DIST_DIR="$release" NODE_OPTIONS=--max-old-space-size=3072 bun run build) > /tmp/hal-account-ads-build.log 2>&1
