...
release=.next-release-20260914yoy01
test ! -e "frontend/$release"
old_target=$(readlink frontend/standalone-server.js)
mapfile -t files < <(tar -tf /tmp/hal-yearly-compare-20260914.tar)
mkdir -p /tmp/hal-yearly-compare-backup-20260914
for path in "${files[@]}"; do
  if test -f "$path"; then cp --parents "$path" /tmp/hal-yearly-compare-backup-20260914/; fi
done
printf '%s\n' "$old_target" > /tmp/hal-yearly-compare-backup-20260914/old-target.txt
tar -xf /tmp/hal-yearly-compare-20260914.tar
...
(cd frontend && NEXT_DIST_DIR="$release" NODE_OPTIONS=--max-old-space-size=3072 bun run build) > /tmp/hal-yearly-compare-build.log 2>&1
