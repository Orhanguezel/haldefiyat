...
release=.next-release-20260915ae01
test ! -e "frontend/$release"
old_target=$(readlink frontend/standalone-server.js)
mapfile -t files < <(tar -tf /tmp/hal-account-logout.tar)
mkdir -p /tmp/hal-account-logout-backup-20260915
for path in "${files[@]}"; do
  if test -f "$path"; then cp --parents "$path" /tmp/hal-account-logout-backup-20260915/; fi
done
printf '%s\n' "$old_target" > /tmp/hal-account-logout-backup-20260915/old-target.txt
tar -xf /tmp/hal-account-logout.tar
restore_link() { ln -sfn "$old_target" frontend/standalone-server.js; }
trap restore_link ERR
(cd frontend && NEXT_DIST_DIR="$release" NODE_OPTIONS=--max-old-space-size=3072 bun run build) > /tmp/hal-account-logout-build.log 2>&1
