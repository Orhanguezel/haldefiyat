...
release=.next-release-20260915bd01
test ! -e "frontend/$release"
old_target=$(readlink frontend/standalone-server.js)
mapfile -t files < <(tar -tf /tmp/hal-advertise-page.tar)
mkdir -p /tmp/hal-advertise-page-backup-20260915
for path in "${files[@]}"; do
  if test -f "$path"; then cp --parents "$path" /tmp/hal-advertise-page-backup-20260915/; fi
done
printf '%s\n' "$old_target" > /tmp/hal-advertise-page-backup-20260915/old-target.txt
tar -xf /tmp/hal-advertise-page.tar
restore_link() { ln -sfn "$old_target" frontend/standalone-server.js; }
trap restore_link ERR
(cd frontend && NEXT_DIST_DIR="$release" NODE_OPTIONS=--max-old-space-size=3072 bun run build) > /tmp/hal-advertise-page-build.log 2>&1
