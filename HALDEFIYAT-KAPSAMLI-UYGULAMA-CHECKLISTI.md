...
- [x] F9.5 `BLOCKED-EXTERNAL`: Hata taraması ve ilk Web Vitals/lab/search/call KPI bazı alındı; 24–72 saat gerçek gözlem penceresi takvim ve gerçek trafik bağımlılığıdır, kodla hızlandırılamaz.
...
- [x] D15 `BLOCKED-EXTERNAL`: Redirect/canonical/sitemap canlı göçü doğrulandı; yalnız Google hesabında write-scope token olmadığı için Search Console submit `403`.
...
- [X] E47 Haftalık rapor üreticisi yayın konseptine taşındı (2026-08-18, Codex yok — Claude uyguladı). 6 işin tamamı canlıda: endeks-öncelikli başlık, 60 krk meta title (kesik yok), anlamsız kategori ortalaması → **piyasa genişliği**, tr-TR sayı/tarih biçimi, editoryal HTML iskeleti (kicker/dek/meta + endeks tablosu + top-5 mover tabloları + metodoloji), takip listesi sürekliliği (`hf_analysis_reports.watchlist` JSON; ertesi hafta "Geçen Haftanın Takip Listesi" otomatik). Guardrail'ler: dar taban (≤7 hal) uyarısı, aynı-aile ayrışma tespiti, İ-casefold denetimi. Yeni modüller `report-format.ts` + `report-html.ts`; 173 test / 0 fail (32 yeni). Taslak 1.926 → 5.854 krk. Yayın kapısı (insan onayı) değişmedi. Brief: `docs/codex-briefs/haftalik-rapor-uretici-sablonu.md`.
...
- [X] S2 **Mersin/Erdemli günlük limon piyasası sayfası.** TAMAMLANDI (2026-08-19, commit b7838c6f+eeffcadd, canlı doğrulandı): `haldefiyat.com/piyasa/erdemli-limon` — yeni `/piyasa/[slug]` config-tabanlı şablon ailesi (`frontend/src/lib/piyasa.ts`; nar/mandalina eklemek = config'e kayıt). İçerik: günlük otomatik piyasa yorumu (hal-başı medyan + 7 gün kıyas + en düşük/yüksek şehir), şehir karşılaştırma tablosu, 90 gün grafik, varyant tablosu (Mayer/yatak), sezon takvimi, FAQ JSON-LD. HKS ulusal "Türkiye" kaydı şehir kıyasından çıkarıldı (çift sayım). Girişler: /urun/limon iç link kartı + sitemap daily/0.85 + IndexNow bildirimi (200). Orijinal kapsam: Ayda 11.000+ gösterimli "mersin limon / erdemli limon piyasası" sorgu ailesi neredeyse tıksız; "günlük" varyantı %6,3 CTR ile talebin şeklini gösteriyor → canlı veri + günlük tek cümle yorumlu KALICI şehir-ürün sayfası (haber makalesi değil). Ham veri: `scripts/seo/limon-opportunity-data.mjs` + `reports/limon-fiyatlari-2026-mersin-erdemli-piyasa-analizi.html`. Şablon nar/mandalina'ya genişleyecek şekilde kurulur.
- [x] S3 **Nar sezon açılışı analizi.** YAZILDI + YAYIN ZAMANLANDI (2026-08-19): rapor id 25, slug `nar-fiyatlari-2026-sezon-acilisi-analizi`, kalite 82/100 (içerik 90). 1.536 kayıt/15 hal verisiyle: canlı sezon tablosu (60–425 TL makası), 12 aylık medyan seyri (depo narı 178,58 zirvesi → erkenci 141,59), sanayi-sofralık ayrımı, eylül göstergeleri. YoY bilinçli olarak eğilim diliyle verildi (yıllık kıyas veri engeli). **Yayın: 20 Ağu 08:00 TR** (hf_scheduled_publishes + */10 cron, IndexNow otomatik). Yayın sonrası canlı kontrol edilince [X] yapılır.
- [x] S4 **Turşuluk sezonu rehberi.** YAZILDI + YAYIN ZAMANLANDI (2026-08-19): rapor id 26, slug `tursuluk-sezonu-2026-tursu-sepeti-hal-fiyatlari`, kalite 90/100 (içerik 100, SEO 100). 10 ürünlük turşu sepeti tablosu (her satır ürün sayfasına iç link — indexli salatalik-tursuluk + domates-yesil-tursu dahil), kayıtlardan alım takvimi (kornişon ağustosta 8→46-48 TL deseni; yeşil domates ekim-kasım 6-7 TL dibi), örnek sepet maliyeti (≈289 TL), ince-taban dürüstlük notu (kornişon/yeşil domates tek hal). **Yayın: 21 Ağu 08:00 TR** (nar 20'sinde, turşuluk 21'inde — günde bir makale). Yayın sonrası canlı kontrolle [X]. Not: S3 nar taslağı da aynı kalite düzeltmeleriyle 82→90'a çıkarıldı.

- [x] S14 **Sezon rehberleri ailesi (/rehber) + ana sayfa bölümü.** (Orhan talebi 2026-08-19 akşam: "turşuluk rehberini ana sayfaya koyalım; konserve/reçel rehberleri de ekleyelim; geçmiş-şimdi grafikleriyle en uygun zamanı bulalım.") YAPILDI (commit 24a0b044 + ince-taban fix): `/rehber` indeksi + 3 rehber (`tursu`, `salca-konserve`, `recel`) — her sepet ürünü için son 12 ayın hal medyanı CSS bar grafiği, kayıttan "en düşük ay" (≥3 hal tabanlı aylardan seçilir) ve bugünkü fiyatın dibe oranından uygun/orta/pahalı rozeti. Ana sayfa (desktop+mobil) SeasonGuides bölümü, sitemap weekly/0.8, turşuluk analizi (id 26) rehbere link'li. Yeni rehber = `frontend/src/lib/rehber.ts` config kaydı. GÜNCELLEME (aynı akşam, Orhan görsel bug bildirimi + çift-yıl talebi): (1) bar yüzde-yüksekliği auto-flex'te çöküyordu → piksel yükseklik (b395578d); (2) grafik ÇİFT SERİYE çevrildi — geçen yıl açık ton + bu yıl koyu ton, Oca–Ara takvim hizalı, 730d aylık veri; (3) rozet/en-uygun-dönem SON 12 AYIN ≥3 hallik dibinden hesaplanır (24 aylık nominal dip enflasyonla her ayı 'pahalı' gösteriyordu). Ardından (Orhan geri bildirimleri): yıllar ayrık renge çevrildi (2025 nötr gri, 2026 marka yeşili; en düşük kayıt halkalı, güncel ay sütunu zeminli, 4 öğeli lejant) ve başlığa CANLI VERİ puls rozeti + FreshnessBadge (son veri tarihi, overview.lastSourceDate) eklendi. Playwright ekran görüntüsüyle canlı doğrulandı. AÇIK: yayınlar canlıya düşünce (20-21 Ağu) rehber+piyasa+analiz URL'lerine toplu IndexNow; rehber sayfalarının GSC etkisi S11'de ölçülür.

- [X] S15 **Sesli arama + arama genişletme (QuickEcommerce kalıbı aktarımı).** (Orhan talebi 2026-08-19 gece.) TAMAMLANDI, canlıda Playwright ile doğrulandı: `useVoiceSearch` hook'u (Web Speech API tr-TR, cross-vendor, Türkçe hata haritası; QE'den farklı olarak interimResults AÇIK — konuşurken canlı transcript), arama modalında mikrofon butonu (dinlerken kırmızı pulse) + "Dinleniyor" bandı, `cleanVoiceQuery` dolgu-kelime temizliği ("domates fiyatları bugün kaç lira"→"domates"), **0-sonuç fallback yazılı aramada da aynı yoldan** (cümle yazan kullanıcı da ürün bulur — canlı test: 0→5 sonuç), son aramalar (localStorage 5 kayıt, silinebilir çip). Yeni analytics event'i eklenmedi (backend enum'una dokunulmadı). Kaynak analiz: QE `customer-web-nextjs/src/components/layout/search-bar.tsx`.

### 16.3 Teknik borç

- [X] S5 **Audit rollup tablosu.** — **TAMAMLANDI 2026-08-31, canlida dogrulandi.** Overview soguk sorgusu **175-242 sn → 0,44-0,88 sn** (~250-400x). Rakamlar ham veriyle **%0,01 icinde** (kalan fark canli trafigin akmasi). **Yapilan:** `094_audit_daily_rollup.sql` (audit_daily_metrics / audit_daily_ips / audit_daily_dimensions) + shared `analytics/rollup.ts` (gun bazinda idempotent ozetleyici) + `repo-overview-rollup.ts` (okuma yolu). Gun x IP tablosu ham logdan **196 kat kucuk** (15.936 -> 51.063 satir vs 3,1M); aralik-genelinde DISTINCT IP, donen ziyaretci ve B2B segmenti oradan hesaplaniyor. analytics-warm cron'u ONCE ozeti tazeliyor sonra cache'i isitiyor. **Diger siteler etkilenmez:** `hasRollupTables()` + `rollupCoversRange()` kapisi — ozet tablosu yoksa veya pencereyi tam kapsamiyorsa ham yol aynen calisir; ozet yolu istisna atarsa da ham yola duser. Ozet tablolari yalniz `hal_fiyatlari` semasinda. **Yol boyunca elenen yanlis hipotez:** yavaslik BOT_UA_SQL regex'i degildi (`realVisitorWhere` zaten is_bot kolonunu kullaniyor); darbogaz I/O — `GROUP BY ip` regex OLMADAN 104 sn suruyor. **Yakalanan gercek hata:** ilk surumde `audit_daily_ips` yalniz sayfa-goruntuleme satirlarini aliyordu, tekil ziyaretci %67 dusuk cikiyordu (14.552 vs 44.741) — yayindan ONCE dogrulama sirasinda bulundu ve duzeltildi. **ACIK KALAN:** retention ucu hala 78 sn (ayri sorgu yolu, bu turda dokunulmadi).
- [~] S6 **deploy.sh admin-build OOM koruması.** Turbopack admin build'i (2,2 GB) 7,9 GB RAM kutuda iki kez OOM'landı (ikincisinde pm2 daemon dahil düştü, ~30 sn 502). Yapısal öneri: admin build adımına `NODE_OPTIONS=--max-old-space-size` sınırı VEYA build sırasında 1 frontend worker'ı geçici durdurup sonra geri açma. Kural şimdiden geçerli: admin build'i art arda tekrar DENEME; admin'de değişiklik yoksa düşen admin adımı zararsız. **GÜNCELLEME 2026-08-31 — ölçüldü, maliyeti belli:** 19–30 Ağustos'ta 463 adet 5xx'in **456'sı dört dağıtım gününde** oluştu (19 Ağu 237, 24 Ağu 103, 30 Ağu 106, 27 Ağu 10), **kalan sekiz günde toplam 7**. 30 Ağustos'taki 19 ardışık fotoğraf batch dağıtımı tek başına 106 5xx + 810 vazgeçilen istek (499) üretti — dönemin 499 zirvesi. Yani altyapı stabil, kesintinin tamamı prosedürden geliyor. **Ek madde A3.3 (kod değil, prosedür):** ardışık içerik dağıtımları tek commit + tek dağıtımda toplanır; 30 Ağustos'taki 19 ayrı dağıtım tek seferde yapılabilirdi. **Ek madde A3.4:** `deploy.sh` sonunda dağıtım penceresindeki 5xx sayısını logdan sayıp çıktıya yaz — regresyon sessiz kalmasın. Kabul: sonraki 3 dağıtım gününde toplam 5xx < 20 (bugünkü dağıtım günü ortalaması 114).
- [x] S7 **Kayseri ETL kaynağı.** AngularJS sayfa, AJAX endpoint reverse-engineering gerekli (browser Network sekmesi ile). 1.177 gösterimlik "kayseri hal fiyatları" sorgu ailesi veri beklediği için içerik tarafı da bu kaynağa bağlı.
- [X] S8 **Haftalık WhatsApp köprüsü.** — **TAMAMLANDI 2026-08-31**: `buildWhatsappWeeklyText()` + `publishWhatsappWeeklyDraft()` eklendi (whatsapp-channel/publisher.ts). Cron'a `runScheduledPublishJob` icinden baglandi ve YALNIZ rapor gercekten yayinlandiginda tetikleniyor (is */10 kosuyor; kosulsuz baglanti her turda taslak gonderirdi). Ozet ilk cumleye kirpiliyor — WhatsApp'ta uzun mesajin sonu 'devamini oku' arkasinda kalip link gorunmuyor. 8 gunden eski rapor duyurulmuyor (cron kacirilirsa eski rapor 'yeni' gibi paylasilmasin).
...
- [X] S11 **26 Ağustos:** ÖLÇÜLDÜ (2026-08-31, `reports/analiz-19-30-ağustos-2026.pdf`). CTA ölçümü ÇALIŞIYOR: 12 günde 1.946 `/track/cta` isteği (1.942 × 204), `home_mobile` ilk kez veri üretti (42 gösterim), enum fix teyitli. Ama **huni geriledi**: newsletter CTA gösterim 757 → 636, kayıt 6 → 2, dönüşüm %0,79 → %0,31. WhatsApp CTA 4 tıklama. 404/index eğrisi: 404 GERİLEMEDİ, ikiye katlandı (1.339 → 2.609 eşleştirilmiş pencerede) — kök neden bulundu, aşağıda S16.
- [X] S12 **2 Eylül:** İKİ GÜN ERKEN ÖLÇÜLDÜ (2026-08-31, `reports/yenileme-etki-analizi-2026-08-31.pdf`). Yenilemenin kullanıcı etkisi **ikinci haftada göründü** ve üç bağımsız sistem doğruluyor — kendi beacon'ımız +%33,9, GA4 +%18,1, GSC organik tıklama +%18,2 (hepsi 15–18 Ağu → 22–25 Ağu eşleştirilmiş pencere). Organik: 11 günlük eşit pencerede tıklama +%10,5, aylık günlük ortalama Temmuz 388 → Ağustos 521 (+%34,2), kazanç pozisyondan değil CTR'den. Hız hedefi AŞILDI: mobil 57 → 91, TBT 1.910 → 0 ms, CrUX LCP artık FAST. **Karşı bulgu:** GA4 davranış metrikleri geriledi (etkileşim %67 → %56,6, hemen çıkma %33 → %43,4, yeni kullanıcı payı %88,6) — erişim büyüyor, elde tutma değil.
...
### 16.6 Oturum kuyruğu — 2026-08-31 (31 Ağustos analiz turunun ürettiği işler)

> **Uygulama detayı ayrı dosyada:** [`HALDEFIYAT-AKSIYON-CHECKLISTI-2026-08-31.md`](./HALDEFIYAT-AKSIYON-CHECKLISTI-2026-08-31.md)
> — her madde için kanıt tablosu, doğrulanmış dosya yolları, adım adım yapılacaklar, sayısal
> kabul kriteri, doğrulama komutu ve tuzaklar. Bu bölüm tek satırlık yürütme kuyruğudur;
> madde kapanınca kanıt **her iki dosyaya da** işlenir.
> **Eşleme:** A1→S16 · A2→S17 · A3→S6 · A4→S25 · A5→S18 · A6→S19 · A7→S20 · A8→S21 ·
> A9→S26 · A10→S22 · A11→S27 · A12→S28 · A13→S23

- [X] S16 **P0 — Kopya `-2` slug'ları için 301 kuralı.** 404 kuyruğunun kök nedeni bulundu: 12 günde 1.765 `-2` son ekli 404, **%99,5'i Googlebot**. Örnek `/firma/3208-haktan-komisyon-evi-2` → 404 iken kanonik `/firma/3208-haktan-komisyon-evi` → 200; ürün tarafında `/urun/elma-2` → 404, `/urun/elma` → 200. Sebep: birleştirme turunun sildiği, benzersiz-slug çakışmasından `-2` eki almış kopya kayıtlar. Sitemap temiz (394 URL, sıfır `-2`), yani sorun tarihsel — ama link değeri her taramada 404'te yanıyor. **Kural:** `{slug}-2` istendiğinde `{slug}` kaydı VARSA ve `-2` kaydı YOKSA kanonik adrese 301. Hâlâ yaşayan 3 istisna kural dışı: `2971-huzur-sebze-ve-meyve-komisyon-evi-2`, `591-yesil-silvan-2`, `domates-2`. Zaman çizgisi: /firma `-2` hataları 24 Ağu'ya kadar günde 0–2, 25 Ağu'da 406. **Uygulama yeri (doğrulandı):** düzeltme route katmanında yapılır — `frontend/src/app/[locale]/(public)/urun/[slug]/page.tsx` (~satır 271, `notFound()` öncesi; canonicalSlug 301'i satır 275'te zaten var) ve `frontend/src/app/[locale]/(public)/firma/[slug]/page.tsx` (~satır 104, `fetchFirm` null dönünce). Ortak yardımcıya alınmalı (kod tekrarı yasağı). **Yan bulgu — ayrıca değerlendir:** `hf_redirects` tablosu sayfa yönlendirmesinde KULLANILMIYOR; frontend'de yalnız `app/sitemap.ts` okuyor, yani tabloya girilen 301'ler sayfa isteğinde uygulanmıyor. Ya route katmanına bağlanmalı ya kapsamı belgelenmeli (S16'yı bloke etmez). **Tuzak:** `410` KULLANMA — `upsertRedirects` 410'da `deactivateGoneProducts()` tetikleyip ürünü pasife çeker (`modules/redirects/repository.ts:116-123`); burada hedefi olan bir 301 var. — **TAMAMLANDI 2026-08-31** (commit 9eb3b8d3, canli dogrulandi): 10/10 test slug'i 301 veriyor (`/urun/kiraz-2` → `/urun/kiraz` vb.), yasayan 3 istisna 200 kaldi. **Duzeltmenin yeri degisti:** sayfaya konunca HIC calismadi — `src/proxy.ts` var-olmayan slug'i sayfadan once ele aliyor (dosyadaki not bunu zaten soyluyormus). `productFallbackRedirect` icine tasindi. Yaniltici olan: `/urun/patates-2` zaten 301 donuyordu ama proxy'nin MEVCUT on-ek kuralindan (`patates-2-kalite`), benim kodumdan degil. **Yan bulgu:** bos katalog artik 404 degil hata firlatiyor — gecici backend kesintisi kalici onbelleklenmis 404 imal ediyordu.
- [X] S17 **P0 — Menü/altbilgi bağlantılarında prefetch'i kapat.** İlk kez ölçüldü: insan sayfa isteklerinin **%91,3'ü RSC prefetch** (12 günde 475.582 prefetch / 45.117 gerçek yükleme). Uç örnekler: /harita 15.810 prefetch / 43 gerçek (368:1), /uyarilar 496:1, /gizlilik-politikasi 502:1. Günde ~39,6 bin karşılıksız istek. `prefetch={false}` ile sunucu yükü bir mertebe düşer; ayrıca "en çok ziyaret edilen sayfa" ölçümü dürüstleşir (önceki raporların sayfa tabloları bu yüzden hatalıydı). **Dosyalar (doğrulandı):** `frontend/src/components/Footer.tsx`, `components/PolicyLinks.tsx`, `components/header/TopbarClient.tsx`. `grep -rn "prefetch" frontend/src --include=*.tsx` bugün HİÇBİR sonuç vermiyor — tüm `<Link>` Next.js varsayılanıyla çalışıyor. **Prefetch KALACAK yerler:** ürün kartları, fiyat listesi satırları, hal kartları, arama sonuçları — kullanıcının gerçekten tıkladığı yollar; oralarda kapatmak LCP'yi geri bozar. Kabul: 3 gün sonra prefetch payı %70 altı VE gerçek sayfa yüklemesi/pageview düşmemiş. — **KOD CANLI 2026-08-31** (commit 99d2127e): Header nav + dropdown + topbar + Footer + PolicyLinks = 14 baglantida `prefetch={false}`. Urun karti, fiyat listesi, hal karti ve arama sonuclarinda prefetch KALDI (kullaniciya hiz katiyor). **Kabul olcumu 3 Eylul'de:** prefetch payi %91,3 → %70 alti VE gercek sayfa yuklemesi dusmemis olmali.
- [X] S18 **P1 — /analiz başlık + açıklamalarını yeniden yaz.** GSC: 8.022 gösterim → 91 tıklama = **%1,13 CTR**, site ortalamasının (%3,09) üçte biri. İçerik aranıyor, görünüyor, tıklanmıyor. Yeni dikeylerde daha keskin: /borsa + /et-fiyatlari + /canli-hayvan-fiyatlari toplam 1.855 gösterim → 10 tıklama (%0,54). En ucuz organik kazanç burada. — **YENIDEN TESHIS + DUZELTILDI 2026-08-31.** Premis yanlisti: meta alanlari 21 raporun hepsinde DOLU ve /analiz ortalama pozisyonu 7,3 — /firmalar (7,5) ile ayni, ama onun CTR'si %4,39, analizinki %1,08. Pozisyon sorunu degil. GSC sorgu×sayfa kirilimi gercek sebebi verdi: **tarihli analiz makalesi gunluk piyasa sayfasini yiyor** ('mersin limon fiyatlari': makale 1.035 gosterim/%0,58, piyasa 189/%1,06; 'erdemli limon piyasasi': makale %0,50, piyasa %2,53). `findPiyasaForArticle()` + makale basina canli-fiyat koprusu eklendi (6 test). Kabul: 4 hafta sonra /piyasa payi toparlaniyor mu.
- [X] S19 **P1 — Bülten teklifini yeniden yaz.** S11'deki gerileme: 636 gösterimde 8 focus, 2 kayıt. Sorun görünürlük değil vaat. GA4 tarafı destekliyor: forma başlayanın %72,7'si tamamlıyor — darboğaz forma gelen kişi sayısı. — **CANLI 2026-08-31.** Kok sorun metin kalitesi degil **vaat uyusmazligiydi**: CTA 'Fiyat Alarmi Kur — hedef fiyata gelince bildirim' diyordu ama form sadece bultene abone ediyor. Artik gercek urunu anlatiyor (pazartesi sabahi, haftanin en cok degisen urunleri, ucretsiz). `mobile_home_sticky` (324 gosterim, 0 kayit) da degistirildi. Zaten dogru metne sahip ve donusum ureten 2 yuzey (price_list_strip, live_price) KONTROL GRUBU olarak dokunulmadi. Kabul: 14 Eylul'de donusum %0,31 → %0,79 uzeri.
- [~] S20 **P2 — Şehir–ürün kesişim sayfalarını çoğalt.** GSC örüntüsü net: "malatya hal komisyoncuları" %47,83 CTR (poz 1,0), "polatlı soğan fiyatları" %27,12; genel "limon fiyatları" %1,64. `/piyasa/[slug]` şablonu (S2) bu iş için zaten kurulu — config'e kayıt eklemek yeterli. — **ERTELENDI 2026-08-31 — veri desteklemiyor.** Adana limon en buyuk boslukmus gibi gorunuyordu (~2.600 gosterim, %0,25-0,42 CTR) ama Adana/Mersin IP-blokta: 30 gunde SIFIR Adana limon kaydi var → verisiz bolge sayfasi ince icerik olurdu. Ikinci limon bolge sayfasi ayrica S18'de yeni duzeltilen kannibalizasyonu artirirdi. Olculen diger 'firsat' sorgulari (kayseri hal 1.098/%0,73, ankara hal 1.072/%1,87) yeni sayfa degil MEVCUT hal sayfalari; baslik/aciklamalari Bursa (%6,33) ile birebir ayni kalipta ve pozisyonlar yakin → fark SERP rekabetinden. **Dogru sira:** S18 koprusunu olc (4 hafta), sonra modeli cogaltmaya karar ver.
- [X] S21 **P2 — `/firmalar?…` parametreli URL'leri kanonikleştir.** 60'tan fazla varyant (`?city=mersin&type=komisyoncu&view=list&page=4` gibi) ayrı ayrı gösterim alıyor; sitemap 394 URL bildirirken 439 sayfa arama sonuçlarında görünüyor. Tarama bütçesi israfı. — **CANLI 2026-08-31** (dogrulandi): `?page`/`?view`/`?q` → `noindex, follow`; `?city&type` → canonical kalici yola (`/firmalar/mersin/komisyoncu`); temiz `/firmalar` indekslenebilir kaldi.
- [X] S22 **P2 — Sessiz ETL başarısızlıklarını hata say.** 10 kaynak `status=ok` dönüp `rows_inserted=0` üretiyor (antkomder ×3, çorum, 6 TOBB borsası) — sağlık raporu bunları sorunlu göstermiyor, "708 run / 5 hata" rakamı bu yüzden iyimser. Ayrıca `tekirdag_resmi` 29 gündür veri yok, `polatli_borsa` dönemde 3× HTTP 500. — **CANLI 2026-08-31** (VPS'te dogrulandi): `etl-health.sh`'a 'SESSIZ BASARISIZLIK' bolumu eklendi (`status=ok` + `rows_inserted=0`). Rapor artik 7 kaynagi '⚠ INCELE' gosteriyor (corum_resmi, balikesir_resmi, polatli_borsa, 4 TOBB borsasi) ve `ETL_HEALTH_IGNORE_EMPTY_SOURCES` muafiyetlerini AYRI, gorunur sekilde listeliyor. **Acik kalan:** partial oraninin %11→%24 sicramasi henuz incelenmedi.
- [X] S23 **P3 — Karantina sıçramasını açıkla.** `hf_price_quarantine` toplam 552 kaydın **427'si (%77) bu 12 günde** düştü. Yeni kalite kuralı mı devreye girdi, yoksa bir kaynak mı bozuldu — sebep kırılımına bakılmalı. — **COZULDU 2026-08-31 — kaynak bozulmasi DEGIL.** `hf_price_quarantine` tablosunun ilk kaydi 2026-08-15: kural o gun devreye girmis, dolayisiyla '552 kaydin %77'si bu donemde' beklenen davranis. 4 reason_code, 11 kaynaga yayilmis. 63 'critical' PRODUCT_UNIT_MISMATCH'in tamami `koli` birimli ve deviation_ratio ≈ 1,0 — yani fiyatlar emsalleriyle uyumlu, isaret birim siniflandirmasi. **Kalan operasyonel bosluk:** 552 kaydin tamami `pending`, hicbiri incelenmemis.
- [ ] S25 **P1 — İlk dış reklamvereni kazan (Orhan + Atakan, kod işi değil).** 12 günde 35.447 banner gösterimi, 36 tıklama (%0,10 CTR), **0 TL gelir, 0 dış ücretli reklamveren**. 14 slotun 7'si boş; dolu 7'sinin hepsi kendi markamız (VistaSeeds ×4) veya Hostinger affiliate ×3. `hf_ad_self_service_requests`, `hf_ad_waitlist`, `hf_ad_payments` tabloları **tamamen boş**. Malzeme 19 Ağustos'tan beri hazır (`reports/haldefiyat-medya-kiti-2026-08.pdf` + canlı indirme + /reklam-ver canlı rakam şeridi) ama gönderilmedi. Eksik olan tek şey temas. Teklifte öne çıkarılacak pozisyonlar: `analiz_sidebar` %0,61 · `prices_top` %0,58 · `home_mid` %0,32 — **uyarı:** üçü de 1.000 gösterimin altında, "erken dönem göstergesi" olarak sunulmalı, garanti olarak değil. Kabul kriteri: `hf_ad_payments`'ta en az 1 `paid`/`partial` kayıt. (S10'un devamı.)
- [X] S26 **P2 — Süresi dolmuş ilanları yenile veya bölümü gizle.** 5 ilanın **hepsi `expired`**, `approved` durumda **0** ilan var — canlı sitede gösterilecek tek aktif ilan yok. Dönemde 0 yeni ilan, 0 ilan sorusu, 0 arama talebi. Buna karşılık /ilan* bölümü 4.517 sayfa isteği alıyor ve menüde duruyor. Önce **karar** gerekiyor (Orhan): modül yaşayacak mı? Yaşayacaksa `valid_until` uzatılır + ilan girişi firma sahiplenme funnel'ına bağlanır; yaşamayacaksa menüden ve ana sayfadan kaldırılır. Boş bölüm ziyaretçiye "burası ölü" mesajı veriyor. Kabul: ya ≥3 aktif ilan, ya bölüm gezinmeden tamamen kalkmış. — **CANLI 2026-08-31.** Karar: modul ve gezinme YERINDE KALDI — `/ilanlar` bos durumu zaten bilincli bir ilan-verme daveti, silinmemeli. Asil sorun bos sayfanin indekslenmesiydi: aktif ilan yokken artik `noindex, follow`. Aktif ilan gelince kendiliginden indekslenir. Orhan karari yalniz 'modul yasayacak mi' sorusu icin acik.
- [X] S27 **P2 — Bayat ve arızalı ETL kaynaklarını onar.** `tekirdag_resmi` son veri **2026-08-01 (29 gün)** — ID-bazlı sayfa, `date:"id:NNN"` backfill formatı mevcut; listeleme sayfası mı değişti, ID mi ilerlemiyor tespit edilmeli. `polatli_borsa` dönemde **3× HTTP 500** (20, 24, 26 Ağu — `bulten.polatliborsa.org.tr`); geçici mi kalıcı mı, kalıcıysa yeni endpoint veya devre dışı. `balikesir_resmi` 9 gün (2 adımlı CSRF, bilinen açık). `bolu_resmi` 16 gün — haftalık kaynak olduğu doğrulanıp bayat listesinden muaf tutulmalı. Kabul: tekirdağ son 7 günde ≥1 gün veri üretmiş, polatlı ardışık hata serisi kırılmış. — **TAMAMLANDI 2026-08-31.** `tekirdag_resmi` devre disi (defaultEnabled false): kaynak site 1 Agustos'tan beri erisilemez — DNS cozuluyor ama VPS + lokal HTTP 000, Scrapling uzerinden HTTP 500. Bize-ozel blok degil, sunucu kapali. Bu arada ETL her gun ayni 1 Agustos anlik goruntusunu isleyip 'partial' uretiyordu (14 gunde 630 rows_inserted ama price_history'ye dusen 1 satir). **`polatli_borsa` INCELENDI ve SAGLAM cikti** — uc 200 donuyor, 7 farkli gunde 19 satir uretmis, 3 HTTP 500 geciciymis; aksiyon gerekmiyor. `bolu_resmi` haftalik kaynak, normal.
- [~] S28 **P2 — Sunucu yanıt süresi (TTFB).** Alan verisinde (CrUX p75) kalan **tek zayıf metrik**: TTFB 1.288 ms `AVERAGE`, FCP 2.276 ms `AVERAGE`; LCP/INP/CLS üçü de `FAST`. Laboratuvarda mobil 91, masaüstü 100, TBT 0 ms — yani istemci tarafı çözülmüş, gecikme sunucuda. **Sıra önemli:** önce S17 (prefetch) yapılsın; günde 39,6 bin isteğin kalkması TTFB'yi kendiliğinden düşürebilir, sonra yeniden ölç. Hâlâ `AVERAGE` ise en çok istenen yolların (`/`, `/fiyatlar`, `/urun/[slug]`) sunucu tarafı önbelleklemesi + render-blocking kaynaklar (~730 ms). Kabul: TTFB p75 < 800 ms → genel CrUX `FAST`. **Tuzak:** alan verisi 28 günlük kayan pencere, etki 3–4 hafta gecikmeli görünür. — **BASELINE OLCULDU 2026-08-31.** Lab TTFB **120 ms** (cok iyi), mobil performans 91 (3 kosunun 2'si; dusuk kosular build yuku sirasinda alinmis, yuk oturunca 91'e dondu — regresyon YOK). Alan verisi (CrUX) hala TTFB 1.288 ms AVERAGE ama **28 gunluk kayan pencere** oldugu icin S17'nin etkisi henuz yansimadi. **Yeniden olcum: Eylul sonu.** Acik kalem: render-blocking kaynaklar (~730 ms).
- [ ] S24 **Takvimli — Kasım başı (S13 girdisi):** AI tarama sıçraması KALICI OLMADI. Eşleştirilmiş pencerede 2.373 → 485 (−%80), aynı anda Googlebot 3.143 → 5.519 (+%76). Dönem toplamı 3.570 hit (~298/gün, ~9K/ay), ölçülebilir gelir 0. 30 Ağu denetiminde GEO (üretken arama) skoru **46/100**, geleneksel SEO 90,7. Kasım kararı bu veriyle alınacak.


## 17. Veri ve editoryal düzeltme — 6 Eylül 2026
...
- [~] S29 Market verisi ve K4 aynı gün/birim/eşleme güvenliği, API günlük fiyat düzeltmesi, plan takvimi ve ölçüm revizyonu.
  ...
  A2–A10 sistem düzeltmeleri ve B1/B3–B5 plan düzeltmeleri `75dd7569` + `6ed03fd8` ile canlı; 49 test, deploy 5xx=0, domates API 39 TL/kg / 5 Eylül doğrulandı. Devam turunda A12 sınıflandırma/kalite referansı, B2 ortak kaynak ve taslak kontrolü, C1/C2 yayın envanteri/taslak güvenliği, D1/D2 prova ayrımı ve sinyal etiketleri uygulandı. 241 URL, iki video, Demre/domates pilotu ve ölçüm formları hazır. A11 gerçek üç cron günü, editör görevlendirmesi, tarihli ölçümler ve ticari sonuçlar §7’de açık izlenir. Eski 31 Ağustos bazları güncel kabul edilmez.


## 8 Eylül 2026 — açık işlerin canlı doğrulaması

Ayrıntı ve kanıt: [uygulama raporu](artifacts/checklist-closeout-2026-09-08/README.md).

- **D15:** Search Console sitemap gönderimi **204**; eski write-scope engeli kalkmış.
- **E42:** ayrı GA4 property **538279658**, web akışı **G-YHLL9WK7ML**, alan adı `https://haldefiyat.com`; Admin API **200**. Yeni, mükerrer property açılmadı.
- **F9.5:** 4–6 Eylül GA4 trafik ve 1–7 Eylül nginx gözlem penceresi alındı. Gözlem tamamlandı; bulunan 5xx sorunları S6 altında açık, başarı diye gizlenmedi.
- **S3/S4:** nar ve turşuluk analizleri canlı **200**. **S14:** rehber/analiz/yerel fiyat dahil yedi URL IndexNow **200**; GSC etki takibi ayrı.
- **S7:** Kayseri son veri **7 Eylül**, 1 Haziran sonrası **97 gün / 11.367 satır**, canlı listede 115 ürün. Eski AngularJS/veri yok engeli geçersiz.
- **S20:** Adana genel limon resmî 90 günlük seriyle canlı/indexlenebilir; Mayer 22 gün/noindex. Mersin resmî kaynak hâlâ WAF 403, son kayıt 22 Haziran. Seri koşulları sağlanmadan çoğaltılmayacak.
- **S6:** bellek sınırı, tek dağıtım kilidi ve 5xx sayacı mevcut. 1–7 Eylül gerçek logu kabul hedefinin henüz karşılanmadığını gösteriyor; üç dağıtım günü <20 hata koşulu açık.
- **F0.23/F7.19/D14/S28:** yeni yerel Lighthouse raporu alındı; mobil 52, TTFB 170 ms, CLS 0. anahtarsız PSI 429 tekrarladı. Karşılaştırılabilir bağımsız performans ve saha p75 kabulü açık. Ana sayfa veri fetch önbelleği düzeltildi; gerçek Safari/NVDA/VoiceOver cihaz testleri yapılmış sayılmadı.
- **E14:** bu görevdeki kaynak dosyaları bağımlılıklarıyla hedefli commit’e alındı. Başka çalışmaya ait `web-connection` değişiklikleri korunuyor; onların sahipliği bu görevde kapanmış sayılmaz.

**Gerçek bağımlılıklar:** F1.38/G1.5 resmî adres metni; F7.8/F7.10 fiziksel cihaz; E9 fotoğraf lisans/kaynak arşivi; F10.4–F10.7/S9/S10/S25 insan kararı ve gerçek müşteri/ödeme; S13/S24 Kasım ölçümü; S29 veri/editoryal belgesindeki takvimli ve operasyonel kabuller.
