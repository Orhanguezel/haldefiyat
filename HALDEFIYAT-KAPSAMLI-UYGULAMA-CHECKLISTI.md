...
- [ ] S26 **P2 — Süresi dolmuş ilanları yenile veya bölümü gizle.** 5 ilanın **hepsi `expired`**, `approved` durumda **0** ilan var — canlı sitede gösterilecek tek aktif ilan yok. Dönemde 0 yeni ilan, 0 ilan sorusu, 0 arama talebi. Buna karşılık /ilan* bölümü 4.517 sayfa isteği alıyor ve menüde duruyor. Önce **karar** gerekiyor (Orhan): modül yaşayacak mı? Yaşayacaksa `valid_until` uzatılır + ilan girişi firma sahiplenme funnel'ına bağlanır; yaşamayacaksa menüden ve ana sayfadan kaldırılır. Boş bölüm ziyaretçiye "burası ölü" mesajı veriyor. Kabul: ya ≥3 aktif ilan, ya bölüm gezinmeden tamamen kalkmış.
- [ ] S27 **P2 — Bayat ve arızalı ETL kaynaklarını onar.** `tekirdag_resmi` son veri **2026-08-01 (29 gün)** — ID-bazlı sayfa, `date:"id:NNN"` backfill formatı mevcut; listeleme sayfası mı değişti, ID mi ilerlemiyor tespit edilmeli. `polatli_borsa` dönemde **3× HTTP 500** (20, 24, 26 Ağu — `bulten.polatliborsa.org.tr`); geçici mi kalıcı mı, kalıcıysa yeni endpoint veya devre dışı. `balikesir_resmi` 9 gün (2 adımlı CSRF, bilinen açık). `bolu_resmi` 16 gün — haftalık kaynak olduğu doğrulanıp bayat listesinden muaf tutulmalı. Kabul: tekirdağ son 7 günde ≥1 gün veri üretmiş, polatlı ardışık hata serisi kırılmış.
- [ ] S28 **P2 — Sunucu yanıt süresi (TTFB).** Alan verisinde (CrUX p75) kalan **tek zayıf metrik**: TTFB 1.288 ms `AVERAGE`, FCP 2.276 ms `AVERAGE`; LCP/INP/CLS üçü de `FAST`. Laboratuvarda mobil 91, masaüstü 100, TBT 0 ms — yani istemci tarafı çözülmüş, gecikme sunucuda. **Sıra önemli:** önce S17 (prefetch) yapılsın; günde 39,6 bin isteğin kalkması TTFB'yi kendiliğinden düşürebilir, sonra yeniden ölç. Hâlâ `AVERAGE` ise en çok istenen yolların (`/`, `/fiyatlar`, `/urun/[slug]`) sunucu tarafı önbelleklemesi + render-blocking kaynaklar (~730 ms). Kabul: TTFB p75 < 800 ms → genel CrUX `FAST`. **Tuzak:** alan verisi 28 günlük kayan pencere, etki 3–4 hafta gecikmeli görünür.
