...
- [ ] S5 **Audit rollup tablosu.** `audit_request_logs` 3,9M satır; overview/retention cold sorgular ~200 sn (FORCE INDEX hint'i kazanç vermedi — ölçüldü). Kalıcı çözüm: günlük per-day/per-segment rollup tablosu (yeni CREATE, seed dosyasına — ALTER yok); overview/retention rollup'tan okur, canlı tabloya yalnız bugün için iner. Alternatif/tamamlayıcı: audit-retention cron penceresini 90 güne çekip tabloyu budamak. Bugünkü geçici çözüm (analytics-warm cron */30 + TTL 40 dk) bu iş bitince sadeleştirilir. **GUNCELLEME 2026-08-31 — olculdu, kismen ilerledi, ROLLUP HALA GEREKLI.** Soguk olcum dogrulandi: overview 175 sn, retention 39 sn (backfill sonrasi 242 / 161 sn). Isitma cron'u calisiyor — panel pratikte 25-60 ms, yani kullanici yavasligi GORMUYOR; sorun cache kacisinda. **Yanlis hipotez elendi:** yavaslik `BOT_UA_SQL` regex'i sanilmisti; `realVisitorWhere` zaten `COALESCE(is_bot,0)=0` kolonunu kullaniyor, regex yalnizca `realOnly=false` dalinda. Kalan maliyet `COUNT(DISTINCT ip)` + gunluk gruplama + onceki-pencere alt sorgusu. **YAN BULGU — CIDDI, DUZELTILDI:** `is_bot` kolonu 1.621.518 satirda BAYATTI (6-19 Agustos; bot deseni PowerShell scraper vakasindan sonra genisletilmis ama eski satirlar guncellenmemis). Panel bu yuzden insan trafigini ~7 KAT fazla gosteriyordu (1,86M yerine 260K). 50k'lik partilerle backfill yapildi, uyumsuzluk 0'a indi. **Not:** haftalik trafik raporlari bu tablodan DEGIL nginx logundan uretiliyor — raporlar etkilenmedi. **Kalan is:** rollup tablosu. Okuma yolu `packages/shared-backend` icinde ve 4+ canli siteyi etkiliyor; ayri repo + kendi deploy zinciri istiyor. Kendi oturumunda, tek is olarak yapilmali.
