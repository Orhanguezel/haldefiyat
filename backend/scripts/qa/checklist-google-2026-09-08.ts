...
try {
 ...
 const res=await fetch(`https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(site)}/sitemaps/${encodeURIComponent('https://haldefiyat.com/sitemap.xml')}`,{method:'PUT',headers:h,signal:AbortSignal.timeout(25000)});
...
try {out.indexNow=await submitToIndexNow(['/rehber','/rehber/tursu','/rehber/salca-konserve','/rehber/recel','/analiz/nar-fiyatlari-2026-sezon-acilisi-analizi','/analiz/tursuluk-sezonu-2026-tursu-sepeti-hal-fiyatlari','/fiyat/adana/limon']);}catch(e:any){out.indexNow={error:e.message};}
