...
export const hfPressCampaigns = mysqlTable(
  "hf_press_campaigns",
  {
    ...
    fromEmail: varchar("from_email", { length: 255 }).notNull().default("noreply@haldefiyat.com"),
    replyToEmail: varchar("reply_to_email", { length: 255 }).notNull().default("info@gzlteknoloji.com"),
    ratePerMinute: int("rate_per_minute").notNull().default(4),
    delayMinSeconds: int("delay_min_seconds").notNull().default(15),
    delayMaxSeconds: int("delay_max_seconds").notNull().default(25),
    status:      mysqlEnum("status", ["draft", "active", "completed", "archived"]).notNull().default("draft"),
    scheduledAt: datetime("scheduled_at", { fsp: 3 }),
    sentAt:      datetime("sent_at", { fsp: 3 }),
    approvedPreflightHash: char("approved_preflight_hash", { length: 64 }),
    approvedAt: datetime("approved_at", { fsp: 3 }),
    approvedBy: varchar("approved_by", { length: 64 }),
    approvalSnapshot: text("approval_snapshot"),
    nextSendAt: datetime("next_send_at", { fsp: 3 }),
...
export const hfPressOutreachLogs = mysqlTable(
  "hf_press_outreach_logs",
  {
    ...
    status:       mysqlEnum("status", ["planned", "processing", "sent", "replied", "published", "bounced", "rejected", "failed", "skipped", "uncertain"]).notNull().default("planned"),
    note:         text("note"),
    ...
    smtpHandoffAt: datetime("smtp_handoff_at", { fsp: 3 }),
    sentSubject: varchar("sent_subject", { length: 255 }),
    sentHtml: text("sent_html"),
    sentText: text("sent_text"),
    sentFromEmail: varchar("sent_from_email", { length: 255 }),
    sentReplyToEmail: varchar("sent_reply_to_email", { length: 255 }),
    createdAt:    datetime("created_at", { fsp: 3 }).default(sql`CURRENT_TIMESTAMP(3)`),
  },
  (t) => [
    ...
    uniqueIndex("hf_press_logs_campaign_contact_channel_uq").on(t.campaignId, t.contactId, t.channel),
    index("hf_press_logs_delivery_idx").on(t.status, t.scheduledAt),
  ],
);
...
export const hfCompetitorSerpRuns = mysqlTable(
  "hf_competitor_serp_runs",
  {
    ...
    depth:        int("depth"),
    gscStartDate: date("gsc_start_date"),
    gscEndDate:   date("gsc_end_date"),
...
export const hfCompetitorSerpResults = mysqlTable(
  "hf_competitor_serp_results",
  {
    ...
    actualEngine: varchar("actual_engine", { length: 16 }),
...
export const hfBanners = mysqlTable(
  ...
  (t) => [
    ...
    index("hf_banners_owner_user_idx").on(t.ownerUserId),
