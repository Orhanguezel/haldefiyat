...
export function makeOtpToken(phone: string, userId: string) {
  return createOtpIdentityToken(phone, userId, env.JWT_SECRET);
}

export function verifyOtpToken(token?: string | null): { phone: string; userId: string } | null {
  return readOtpIdentityToken(token, env.JWT_SECRET);
}
...
export async function sendOtp(rawPhone: string, userId: string) {
  ...
  const delivery = await sendListingOtpSms(phone, code);
  if (!delivery.ok) return { ok: false as const, code: 503, error: "sms_unavailable" };
  await db.insert(hfPhoneVerifications).values({
    userId, phone, code: otpDigest(phone, code), purpose: "listing", expiresAt: new Date(Date.now() + TTL_MS),
  });
  return { ok: true as const, phone };
}

export async function verifyOtp(rawPhone: string, code: string, userId: string) {
  const phone = normalizeTrPhone(rawPhone);
  ...
  const [row] = await db.select().from(hfPhoneVerifications).where(and(
    ...
    eq(hfPhoneVerifications.userId, userId),
  ...
  return { ok: true as const, token: makeOtpToken(phone, userId), phone };
}
