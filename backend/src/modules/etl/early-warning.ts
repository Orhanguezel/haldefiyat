...
import { escapeTgHtml, sendTelegramAdminAlert } from "@/modules/alerts/telegram";
...
export async function checkAndNotifyEarlyWarning(): Promise<{ count: number }> {
  ...
  await Promise.all([
    // Urun adinda `&` gecerse HTML ayristirma alarmi tumden dusurur.
    sendTelegramAdminAlert(escapeTgHtml(text)),
