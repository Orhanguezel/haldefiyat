...
describe("canonical product contract", () => {
  ...
  it("normalizes recorded spelling fixtures without merging real variants", () => {
    ...
    expect(scoreProductMatch({ rawName: "Maydonoz", rawUnit: "demet", canonicalName: "Maydanoz", aliases: ["Maydonoz"] }).score).toBe(90);
    expect(scoreProductMatch({ rawName: "Incir", rawUnit: "kg", canonicalName: "İncir" }).score).toBe(95);
    expect(scoreProductMatch({ rawName: "Hındıstan Cevizi", rawUnit: "adet", canonicalName: "Hindistan Cevizi", aliases: ["Hındıstan Cevizi"] }).score).toBe(90);
