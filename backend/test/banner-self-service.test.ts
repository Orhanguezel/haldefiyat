...
describe("reklam portalı sahiplik çözümü", () => {
  test("ilan sahibi firma kaydı olmadan kendi ilan reklamını görür ve bedeli okur", () => {
    const owner = resolveSelfServiceOwner(banner, new Map(), new Map([[36, listing]]));
  ...
  test("firma üyesi finansal yetkisi yoksa bedel alanları gizlenir", () => {
    ...
    const owner = resolveSelfServiceOwner(firmBanner, new Map([[5, false]]), new Map());
  ...
  test("başkasının ilanı ya da firması erişim vermez", () => {
    expect(resolveSelfServiceOwner(banner, new Map([[5, true]]), new Map([[99, { ...listing, id: 99 }]]))).toBeNull();
  });

  test("ilan reklamı görünümü ilan bağlantısını ve bedeli taşır", () => {
    const view = selfServiceCampaignView(banner, resolveSelfServiceOwner(banner, new Map(), new Map([[36, listing]]))!);
