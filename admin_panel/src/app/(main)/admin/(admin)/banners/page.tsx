...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (tab && ["campaigns", "alerts", "revenue", "reports", "calendar", "slots", "waitlist", "packages"].includes(tab))
      setActiveTab(tab);
  }, []);
  ...
  const { data, isLoading, isError } = useListBannersAdminQuery(undefined);
  ...
  return (
    <div className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            {isLoading
              ...
              : t("subtitle", {
                  ...
                  reserved: money(stats.reservedRevenue),
                })}
          </p>
        </div>
        <Button asChild>
        ...
        tiles={[
          {
            ...
            label: t("tiles.total"),
            value: stats.total,
            hint: t("tiles.totalHint"),
            active: activeTab === "campaigns" && filters.lifecycle === ALL,
            onClick: () => {
              patch({ lifecycle: ALL });
              changeTab("campaigns");
            },
          },
          {
            ...
            label: t("tiles.live"),
            value: stats.live,
            hint: t("tiles.liveHint"),
            tone: "text-emerald-600",
            active: activeTab === "campaigns" && filters.lifecycle === "live",
            onClick: () => {
              patch({ lifecycle: "live" });
              changeTab("campaigns");
            },
          },
          {
            key: "scheduled",
            label: t("tiles.scheduled"),
            value: stats.scheduled,
            hint: t("tiles.scheduledHint"),
            active: activeTab === "campaigns" && filters.lifecycle === "scheduled",
            onClick: () => {
              patch({ lifecycle: "scheduled" });
              changeTab("campaigns");
            },
          },
          {
            ...
            tone: stats.open ? "text-amber-600" : "",
            active: activeTab === "campaigns" && filters.lifecycle === "open",
            onClick: () => {
              patch({ lifecycle: "open" });
              changeTab("campaigns");
            },
          },
          {
            key: "endingSoon",
            label: t("tiles.endingSoon"),
            value: stats.endingSoon,
            hint: t("tiles.endingSoonHint"),
            active: activeTab === "campaigns" && filters.lifecycle === "endingSoon",
            onClick: () => {
              patch({ lifecycle: "endingSoon" });
              changeTab("campaigns");
            },
          },
          {
            key: "attention",
            label: t("tiles.attention"),
            value: attention,
            hint: t("tiles.attentionHint"),
            tone: attention ? "text-rose-600" : "",
      ...
      <Tabs value={activeTab} onValueChange={changeTab} className="min-w-0">
        <TabsList className="h-auto w-full min-w-0 justify-start overflow-x-auto [&>[role=tab]]:flex-none [&>[role=tab]]:px-4 [&>[role=tab]]:py-2 xl:[&>[role=tab]]:flex-1">
          <TabsTrigger value="campaigns">{t("tabs.campaigns")}</TabsTrigger>
          <TabsTrigger value="alerts">
            {t("tabs.alerts")}
            {attention ? (
              <span className="ml-1 rounded-full bg-rose-500 px-1.5 text-[10px] text-white">{attention}</span>
            ) : null}
          </TabsTrigger>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Select value={filters.position} onValueChange={(v) => patch({ position: v })}>
              ...
              <SelectContent>
                ...
                {slots.map((s) => (
                  <SelectItem key={s.slotKey} value={s.slotKey}>
                    {s.label}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="flex flex-wrap gap-1.5">
            {CHIPS.map((k) => (
              <Button
                key={k}
                size="sm"
                variant={filters.lifecycle === k ? "default" : "outline"}
                onClick={() => patch({ lifecycle: k })}
              >
                {t(`chips.${k}`)}
              </Button>
            ))}
            <span className="ml-auto self-center text-sm text-muted-foreground">
              {t("table.summary", { count: visible.length })}
            </span>
          </div>
          <CampaignsTable
            rows={visible}
        ...
        <TabsContent value="alerts" className="mt-4">
          <AlertsPanel banners={banners} t={t} tc={tc} />
        </TabsContent>
        <TabsContent value="revenue" className="mt-4">
          <RevenuePanel banners={banners} slots={slots} t={t} />
        </TabsContent>
        <TabsContent value="reports" className="mt-4">
          <ReportsPanel
            ...
            campaignsFailed={isError}
            t={t}
          />
        </TabsContent>
        <TabsContent value="calendar" className="mt-4">
          <CalendarPanel slots={slots} t={t} tc={tc} />
        </TabsContent>
        <TabsContent value="slots" className="mt-4">
          <SlotsPanel slots={slots} availability={availability?.items ?? []} t={t} />
        </TabsContent>
        <TabsContent value="waitlist" className="mt-4">
          <WaitlistPanel slots={slots} t={t} tc={tc} />
        </TabsContent>
        <TabsContent value="packages" className="mt-4">
          <PackagesPanel slots={slots} t={t} tc={tc} />
        </TabsContent>
      </Tabs>
