...
function StatusBadge({ status }: { status: string }) {
  ...
  return <Badge variant={variant} className="font-normal">{STATUS_LABEL[status] ?? status}</Badge>;
}
...
function Validity({ value }: { value: string }) {
  ...
  return (
    <div className="whitespace-nowrap">
      ...
      <div className={`text-xs ${tone}`}>
        {left == null ? '' : left < 0 ? 'süresi doldu' : left === 0 ? 'bugün bitiyor' : `${left} gün kaldı`}
      </div>
    </div>
  );
}
...
export function ListingsTable({ items, ads, busy, onEdit, onModerate, onDelete }: Props) {
  if (!items.length) {
    return (
      <div className="rounded-lg border border-dashed py-16 text-center text-sm text-muted-foreground">
        {busy ? 'Yükleniyor…' : 'Bu filtrede ilan yok.'}
      </div>
    );
  }
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/40 hover:bg-muted/40">
            <TableHead className="w-[45%]">İlan</TableHead>
            <TableHead className="w-24">Tip</TableHead>
            <TableHead className="w-28">Durum</TableHead>
            <TableHead className="w-36">Geçerlilik</TableHead>
          ...
          {items.map((item) => {
            ...
            return (
              <TableRow key={item.id} className="group cursor-pointer" onClick={() => onEdit(item)}>
                ...
                <TableCell className="text-sm text-muted-foreground">{TYPE_LABEL[item.listingType] ?? item.listingType}</TableCell>
                <TableCell><StatusBadge status={item.status} /></TableCell>
                <TableCell><Validity value={item.validUntil} /></TableCell>
