...
export const PUBLICATION_TYPES: PressPublicationType[] = [
  "newspaper",
  "website",
  "association",
  "chamber",
...
export function formatDateTime(value?: string | null) {
  if (!value) return "—";
  const d = new Date(value);
  return Number.isNaN(d.getTime()) ? value : d.toLocaleString("tr-TR", { dateStyle: "medium", timeStyle: "short" });
}
...
export function applyFilters(rows: PressContact[], f: Filters) {
  ...
  return rows.filter((c) => {
    if (
      q &&
      !`${c.organization} ${c.contactName ?? ""} ${c.email} ${c.city ?? ""} ${c.tags.join(" ")}`
