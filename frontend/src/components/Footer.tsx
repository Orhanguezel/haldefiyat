      ...
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="mb-7 grid grid-cols-1 gap-10 border-b border-(--color-border) pb-12 sm:grid-cols-2 lg:grid-cols-[1.7fr_repeat(4,minmax(0,1fr))]">
          ...
          <div className="space-y-4">
            ...
            {(socialTwitter || socialInstagram || socialYoutube || socialTelegram || socialWhatsapp) && (
              <div className="flex flex-wrap items-center gap-3 pt-1">
                {socialTwitter && (
                  <a href={socialTwitter} target="_blank" rel="noopener noreferrer" aria-label="Twitter/X"
                    className="inline-flex min-h-11 min-w-11 items-center justify-center gap-2 rounded-lg border border-(--color-border) px-3 text-(--color-foreground) hover:text-(--color-brand) transition-colors">
                    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L1.254 2.25H8.08l4.259 5.63 5.905-5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                    </svg><span className="text-xs font-semibold">Twitter/X</span>
                  </a>
                )}
                {socialInstagram && (
                  <a href={socialInstagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                    ...
                    </svg><span className="text-xs font-semibold">Instagram</span>
                  </a>
                )}
                {socialYoutube && (
                  <a href={socialYoutube} target="_blank" rel="noopener noreferrer" aria-label="YouTube"
                    ...
                    </svg><span className="text-xs font-semibold">YouTube</span>
                  </a>
                )}
                {socialTelegram && (
                  <a href={socialTelegram} target="_blank" rel="noopener noreferrer" aria-label="Telegram"
                    ...
                    </svg><span className="text-xs font-semibold">Telegram</span>
                  </a>
                )}
                {socialWhatsapp && (
                  <a href={socialWhatsapp} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp Kanalı"
                    className="inline-flex min-h-11 min-w-11 items-center justify-center gap-2 rounded-lg border border-(--color-border) px-3 text-(--color-foreground) hover:text-(--color-brand) transition-colors">
                    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    </svg><span className="text-xs font-semibold">WhatsApp Kanalı</span>
                  </a>
                )}
              </div>
            )}
          ...
          <div className="text-center text-(--color-muted) sm:text-left">
            ...
            {legalEntityName ? (
              <p className="mt-1 text-[12px]">
                ...
                {contactEmail ? <> · <ObfuscatedEmail {...splitEmail(contactEmail)} className="hover:text-(--color-brand)" /></> : null}
              </p>
            ) : null}
          </div>
          <a
