...
function DeferredGoogleTags({
  ...
  return (
    <Script id="google-tags-deferred-loader" strategy="afterInteractive">
      {`(function(c){var loaded=false,timer;function add(src){var s=document.createElement('script');s.async=true;s.src=src;document.head.appendChild(s)}function load(){if(loaded)return;loaded=true;clearTimeout(timer);['pointerdown','keydown','touchstart','scroll'].forEach(function(e){removeEventListener(e,load)});window.dataLayer=window.dataLayer||[];window.gtag=window.gtag||function(){dataLayer.push(arguments)};if(c.gtmId){dataLayer.push({'gtm.start':Date.now(),event:'gtm.js'});add('https://www.googletagmanager.com/gtm.js?id='+encodeURIComponent(c.gtmId))}else if(c.ga4Id){gtag('js',new Date());gtag('config',c.ga4Id);add('https://www.googletagmanager.com/gtag/js?id='+encodeURIComponent(c.ga4Id))}if(!c.gtmId&&c.adsConversionId){gtag('js',new Date());gtag('config',c.adsConversionId,{allow_enhanced_conversions:true});if(c.adsConversionId!==c.ga4Id)add('https://www.googletagmanager.com/gtag/js?id='+encodeURIComponent(c.adsConversionId))}}['pointerdown','keydown','touchstart','scroll'].forEach(function(e){addEventListener(e,load,{once:true,passive:true})});function idle(){if('requestIdleCallback'in window){requestIdleCallback(load,{timeout:2500})}else{timer=setTimeout(load,2500)}}if(document.readyState==='complete')idle();else addEventListener('load',idle,{once:true})})(${config});`}
    </Script>
  );
}
